...
def dataForm(form,data):
    ...
    if form == "listJob":
        return {"version": "2.0","template":{"outputs":[{"simpleText": {"text": "실행가능한 작업 목록입니다.\n" + data + "실행하실 작업을 입력해주세요."}}]}}
    if form == "bottonListMsg":
        return {"version": "2.0","template": {"outputs": [{"basicCard": {"title": "작업 목록","description": "원하시는 작업을 선택해주세요.","buttons": [{"action": "message","label": "작업실행","messageText": "작업리스트"},{"action":  "webLink","label": "작업생성","webLinkUrl": "https://echo.hansol.com/"},{"action":  "webLink","label": "모니터링","webLinkUrl": "https://echo.hansol.com/"}]}}]}}